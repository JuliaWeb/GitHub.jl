...
#
# - A GitHubType's field types should be concrete types, a
#   Vectors of concrete types, Dicts or Null.
...
function Base.:(==)(a::GitHubType, b::GitHubType)
    ...
    for f in fieldnames(typeof(a))
        ...
        if !((af === nothing && bf === nothing) || (af == bf))
            return false
        end
    end
    return true
...
function extract_nothing(data::Dict, key, ::Type{T}) where {T}
    if haskey(data, key)
        val = data[key]
        if !(isa(val, Nothing))
            if T <: Vector
                V = eltype(T)
                return V[prune_github_value(v, V) for v in val]
            else
                return prune_github_value(val, T)
            end
        end
    end
    return nothng
