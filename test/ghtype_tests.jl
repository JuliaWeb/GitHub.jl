...
@testset "Repo" begin
    ...
    repo_result = Repo(
        ...
        nothing,
        nothing,
        URIs.URI(repo_json["url"]),
        nothing,
        nothing,
        URIs.URI(repo_json["ssh_url"]),
        nothing,
...
@testset "Webhook" begin
    ...
    hook_result = Webhook(
        hook_json["id"],
        URIs.URI(hook_json["url"]),
        URIs.URI(hook_json["test_url"]),
        URIs.URI(hook_json["ping_url"]),
        hook_json["name"],
...
@testset "Gist" begin
    ...
    gist_result = Gist(
      URIs.URI(gist_json["url"]),
      URIs.URI(gist_json["forks_url"]),
      URIs.URI(gist_json["commits_url"]),
      gist_json["id"],
      ...
      gist_json["truncated"],
      gist_json["comments"],
      URIs.URI(gist_json["comments_url"]),
      URIs.URI(gist_json["html_url"]),
      URIs.URI(gist_json["git_pull_url"]),
      URIs.URI(gist_json["git_push_url"]),
...
@testset "CheckRun" begin

    in_progress_json = JSON.parse("""
    {
        "name": "mighty_readme",
    ...
    in_progress_result = CheckRun(in_progress_json)

    @test in_progress_result.external_id == "42"
    @test in_progress_result.output.title == "Mighty Readme report"
    ...
    completed_result = CheckRun(completed_json)

    @test completed_result.name == "mighty_readme"
    @test completed_result.output.title == "Mighty Readme report"
    @test completed_result.output.annotations[1].path == "README.md"

    @test completed_result.output.images[1].image_url == "http://example.com/images/42"

    @test completed_result.actions[1].label == "Fix"
...
@testset "License" begin
    ...
    license_result = License(
        ...
        URIs.URI(license_json["url"]),
        URIs.URI(license_json["html_url"]),
